...
export const ContentSearchFilter = () => {
  ...
  return (
    <div className="content-search-filter">
      <div className="content-search-filter__input-wrap">
        <input
          ref={inputRef}
          type="search"
        ...
        {showSuggestions && suggestions.length > 0 && (
          <ul ref={listRef} className="content-search-filter__suggestions">
            {suggestions.map((item, i) => (
              <li key={getContentKey(item)}>
                <button
                  type="button"
                  className={`content-search-filter__suggestion ${
                    i === focusedIndex ? 'content-search-filter__suggestion--focused' : ''
                  }`}
                  onMouseDown={(e) => {
                    e.preventDefault();
                    handleOpenItem(item.id);
                ...
                >
                  <span className="content-search-filter__suggestion-title">{item.title}</span>
                  <span className="content-search-filter__suggestion-type">
