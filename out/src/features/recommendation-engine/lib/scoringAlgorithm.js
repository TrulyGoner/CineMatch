 ...
 */
const buildGenreClickMap = (events) => {
    const genreCounts = {};
 ...
 */
export const calculateRecommendations = (contents, events, weights) => {
    ...
    const scored = contents.map((content) => {
        ...
        const daysSinceRelease = (Date.now() - new Date(content.releaseDate).getTime()) / (1000 * 60 * 60 * 24);
        const freshnessMultiplier = daysSinceRelease < FRESHNESS_DAYS_THRESHOLD ? FRESHNESS_BONUS : 1;
        const freshnessScore = daysSinceRelease < FRESHNESS_DAYS_THRESHOLD ? (weights.freshness / 100) * freshnessMultiplier : 0;
...
export const shuffleRecommendations = (contents) => {
